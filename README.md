...
- 🛡️ **加车前终极规格硬门禁 (`FINAL_TARGET_CHECK`)**：触发加车前最后 1 毫秒执行原子级规格一致性校验，若有任何不匹配立即硬熔断，严防下错订单。
- 🌐 **双通道解耦监控**：
...
- 👤 **浏览器置前与人工安全接管 (Human Handoff)**：加车完成后毫秒级将官方浏览器窗口置前，播放本地高优先级警报音并推送飞书卡片，由用户亲自核对地址与完成支付。
...
## 📊 性能与基准验证 (Controlled Benchmarks)

在本地离线密封测试与受控沙盒环境下，系统的关键响应时延指标如下：

| 关键阶段 | 测量时延 | 说明与保障机制 |
| :--- | :--- | :--- |
| **Catalog 权威解析** | $\approx 10\text{ ms}$ | 本地 JSON 结构化索引缓存，零网络阻塞 |
| **规格保持核验 $\to$ 终极硬门禁** | $\approx 1.13\text{ s}$ | 预选状态完整保持，直接跳过冗余点击快速就绪 |
| **到达人工接管就绪点** | $\approx 1.26\text{ s}$ | 毫秒级唤醒原生窗口置前，播放本地音频警报 |

> ⚠️ **特别说明**：上述耗时数据基于离线受控仿真与演练测试。**在所有基准验证与回归测试中，实际“加入购物袋”与“提交订单”请求均被物理阻断或密封 Mock，绝不向 Apple 发送真实购买交易。**
...
- 登录成功后直接关闭浏览器窗口，会话凭据将加密保存在本地 `./browser_data` 中。
...
- **当开售信号触发时，系统毫秒级核验规格并推进至接管点，随后立即置前浏览器，由您本人亲自完成付款**。
